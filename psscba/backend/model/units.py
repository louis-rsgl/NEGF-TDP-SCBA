...
def time_from_ps(t_ps, gamma_eV: float):
    """Convert the original runner's stored picoseconds to ``hbar/Gamma``."""
    return np.asarray(t_ps) / (1e12 * gamma_to_time_unit_s(gamma_eV))
...
def current_from_uA(I_uA, gamma_eV: float):
    """Convert the original runner's stored microamps to ``e Gamma/hbar``."""
    return np.asarray(I_uA) / (1e6 * gamma_to_current_unit_A(gamma_eV))
