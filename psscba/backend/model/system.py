...
if TYPE_CHECKING:
    from psscba.backend.stationary.scba import InstalledKernelData, Solver, SolverResult
    from psscba.backend.model.reporting import Reporter
    from psscba.backend.poles.minimal import PoleCache
...
class System:
    ...
    square_residue_method: str = "batched_contour"

    verbose: bool = True

    _cached_eq_poles: list | None = None
    _stationary_result: Optional["SolverResult"] = None
    _stationary_solver: Optional["Solver"] = None
    _stationary_kernel: Optional["InstalledKernelData"] = field(
    ...
    _square_cache_store: object | None = field(default=None, init=False, repr=False)
    ...
    def invalidate_stationary_cache(self) -> None:
        self._stationary_result = None
        self._stationary_solver = None
        self._stationary_kernel = None
        self._pole_cache = None
        self._square_kernel_cache = None
        self._square_cache_store = None
    ...
    def solve_stationary(
        ...
        """Solve the protocol-conditioned stationary reference problem."""
        self.validate_pulse_protocol()
        if (
            self._stationary_solver is not None
            and self._stationary_result is not None
            and not force
        ):
            return self._stationary_result
        ...
        result = solver.solve()
        self._stationary_solver = solver
        self._stationary_result = result
        self._stationary_kernel = solver.installed_kernel
        self._pole_cache = None
        self._square_kernel_cache = None
        self._square_cache_store = None
    ...
    def stationary_kernel(self) -> "InstalledKernelData":
        if self._stationary_kernel is None:
            self.solve_stationary()
        if self._stationary_kernel is None:
            raise RuntimeError("SCBA completed without producing an installed kernel.")
        return self._stationary_kernel
    ...
    def prepare_poles(self, force: bool = False) -> "PoleCache":
        ...
        self._pole_cache = build_pole_cache(self, self.stationary_kernel())
        self._square_kernel_cache = None
        return self._pole_cache

    def citations(self) -> list[dict]:
        """Return method and software citation records for this calculation."""
        from psscba.frontend.provenance import citation_records

        return citation_records()

    def provenance(self) -> dict:
        """Return software, runtime, method, units, and numerical settings."""
        from psscba.frontend.provenance import system_provenance

        return system_provenance(self)

    def require_stationary(self) -> None:
        if self._stationary_solver is None or self._stationary_result is None:
            raise RuntimeError(
                "Stationary solution not available. Call solve_stationary() first."
            )

    def G_reference_R(self, e):
        self.require_stationary()
        return self._stationary_solver.GR(e)

    def G_reference_less(self, e):
        self.require_stationary()
        return self._stationary_solver.Gless(e)

    def G_reference_A(self, e):
        self.require_stationary()
        return self._stationary_solver.GA(e)
