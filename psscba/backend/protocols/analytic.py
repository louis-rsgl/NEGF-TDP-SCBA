...
def downward_lead_vertex(sys: System, cache: PoleCache, z, energy, alpha: Lead):
    ...
    return (
        ...
        + (sys.DELTA + lead_fd) * Gbiased_R_mpm(sys, cache, energy + delta_alpha)
    )


def downward_interaction_vertex(sys: System, z, shifted_energy):
    z = np.asarray(z, dtype=np.complex128)
...
def A_down(sys: System, energy, t, alpha: Lead, cache: PoleCache | None = None):
    ...
    tt = times[None, None, :]
    mfac = downward_lead_vertex(sys, cache, x, e, alpha)
    transient = np.sum(
        r * np.exp(-1j * (x - e) * tt)
...
def A_down_direct(
    ...
    delta_alpha = sys.Delta(alpha)
    gbar_boundary = Gbiased_R_mpm(sys, cache, energy + delta_alpha)
...
def B_down(
    ...
    mfac = downward_lead_vertex(sys, cache, cache.xi[:, None], primes[None, :], beta)
    pole_term = np.sum(
        expc(e - x, tt) * r
        * mfac.T[None, :, :, None]
        / (x - ep - sys.Delta(beta)),
...
def C_down(
    ...
    tt = times[None, None, :]
    sc = downward_interaction_vertex(sys, x, ep)
    transient = np.sum(
    ...
    zero_time = times == 0.0
    if np.any(zero_time):
        out[:, zero_time] = Gbiased_R_mpm(sys, cache, shifted)[:, None]
...
def D_down(
    ...
    tt = times[None, None, None, :]
    sc = downward_interaction_vertex(sys, cache.xi[:, None], shifted[None, :])
    pole_term = np.sum(
        expc(e - x, tt) * r
        * sc.T[None, :, :, None]
        * Gbiased_R_mpm(sys, cache, shifted)[None, :, None, None]
        / (x - ep),
        axis=2,
    )
    out = (
...
def upward_lead_vertex(sys: System, cache: PoleCache, z, energy, alpha: Lead):
    z = np.asarray(z, dtype=np.complex128)
    energy = np.asarray(energy, dtype=np.complex128)
...
def upward_interaction_vertex(sys: System, cache: PoleCache, z, shifted_energy):
    z = np.asarray(z, dtype=np.complex128)
    shifted_energy = np.asarray(shifted_energy, dtype=np.complex128)
    finite_difference = np.zeros(
...
def upward_a_residue(
    ...
    *,
    ...
    e = energies[None, :, None]
    tt = times[None, None, :]
    mfac = upward_lead_vertex(sys, cache, x, e, alpha)
    transient = np.sum(
        r
...
def upward_c_residue(
    ...
    *,
    ...
    ep = shifted[None, :, None]
    tt = times[None, None, :]
    nfac = upward_interaction_vertex(sys, cache, x, ep)
    transient = np.sum(
...
def psi_lead_down_batch(sys, cache, outer, inner, time, alpha, A_values):
    ...
    for beta in sys.lead_names:
        ...
        singular_values = (
            ...
            * np.conjugate(Gbiased_R_mpm(sys, cache, inner + db))
        )
        result -= (
...
def _psi_ep_down_batch(sys, cache, frozen, outer, inner, time, alpha, C_values):
    ...
    for omega, occupation_factor in (
        ...
        singular_values = (
            ...
            * np.conjugate(Gbiased_R_mpm(sys, cache, inner + omega))
        )
        result += (
...
def psi_lead_up_batch(sys, cache, outer, inner, time, alpha, A_values):
    result = np.zeros(len(outer), dtype=np.complex128)
    da = sys.Delta(alpha)
    for beta in sys.lead_names:
        db = sys.Delta(beta)
...
def time_derivative(values, times):
    """Fourth-order derivative on a uniform grid, with safe fallbacks."""
    values = np.asarray(values)
    times = np.asarray(times, dtype=float)
    if len(times) < 5:
        return np.gradient(values, times, edge_order=2 if len(times) > 2 else 1)
    steps = np.diff(times)
    if not np.allclose(steps, steps[0], rtol=1e-10, atol=1e-14):
        return np.gradient(values, times, edge_order=2)
    dt = float(steps[0])
    derivative = np.empty_like(values, dtype=np.result_type(values, float))
    derivative[2:-2] = (
        values[:-4]
        - 8.0 * values[1:-3]
        + 8.0 * values[3:-1]
    ...
    first = np.array([-25.0, 48.0, -36.0, 16.0, -3.0]) / 12.0
    second = np.array([-3.0, -10.0, 18.0, -6.0, 1.0]) / 12.0
    derivative[0] = np.sum(first * values[:5]) / dt
    derivative[1] = np.sum(second * values[:5]) / dt
    derivative[-1] = -np.sum(first * values[-5:][::-1]) / dt
    derivative[-2] = -np.sum(second * values[-5:][::-1]) / dt
    return derivative
...
def continuity_residual(currents, occupation, times):
    ...
    return current_sum - time_derivative(occupation, times)
...
def _transient_diagnostics(
    ...
    diagnostics: dict[str, float | int | bool | str] = {
        ...
        "transient_approximation": "installed stationary electron-phonon kernel",
        "N0": frozen.N0,
...
def current_all_square(
    ...
    if observation_times is None:
        ...
        if times.ndim != 1 or len(times) < 2:
            raise ValueError("observation_times must be a one-dimensional grid.")
        if np.any(np.diff(times) <= 0.0) or times[0] < 0.0:
            raise ValueError("observation_times must be increasing and non-negative.")
        n_t = len(times)
    ...
    continuity = continuity_residual(currents, occupation, times)
    switch_index = int(np.argmin(np.abs(times - duration)))
    extra = {
        ...
        "square_turnoff_sampled": bool(np.isclose(times[switch_index], duration)),
