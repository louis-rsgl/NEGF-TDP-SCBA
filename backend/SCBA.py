...
class FrozenSCBA:
    """Immutable stationary boundary from which the pulse kernel is frozen."""
    ...
    @property
    def Gbar_R(self) -> np.ndarray:
        """Legacy alias for the protocol's stationary reference Green function."""
        return self.G_reference_R

    @property
    def Gbar_less(self) -> np.ndarray:
        """Legacy alias for the protocol's stationary reference lesser function."""
        return self.G_reference_less
...
class Solver:
    def __init__(self, sys, w_min: float, w_max: float, n_w: int):
        ...
        self.frozen: Optional[FrozenSCBA] = None
    ...
    def _interp_real_grid(self, values: np.ndarray, x, *, retarded: bool):
        ...
        if np.any(np.abs(x_arr.imag) > 1e-14):
            raise ValueError(
                ...
                "frozen MPM propagator for complex frequencies."
            )
        xr = x_arr.real
    ...
    def solve_weak_born(self) -> SolverResult:
        """Strict O(g^2) frozen kernel built from the no-phonon reference."""
        if self.GR_values is None or self.Gless_values is None:
            self.initialize()
        assert self.GR_values is not None and self.Gless_values is not None
    ...
    def solve(self) -> SolverResult:
        ...
        # Re-evaluate the frozen kernel from the final mixed state.
        kernel_less = np.array(self.Gless_values, copy=True)
        sigma_R, sigma_less, sigma_H, sigma_dyn_R, gamma_ep = (
            self.compute_ep_self_energies()
        )
